...
if __name__ == "__main__":
    ...
    print("=== compliance 0.75 ===")
    r1 = generate_breath_cycles(
        {**RDS_PARAMS, "pressure_support_cmH2O": 10.0}, n_cycles=1)
    ...
    # r = generate_breath_cycles({**RDS_PARAMS, "pressure_support_cmH2O": 10.0}, n_cycles=1)
    # psv.DT = 0.01
    # print(r.get("insp_ended_by_reversal"))
    # psv.DT = 0.001
    # for c in (0.75, 1.5, 3.0, 6.0):
    #     p = {**RDS_PARAMS, "pressure_support_cmH2O": 10.0, "compliance_ml_per_cmH2O": c}
    #     r = generate_breath_cycles(p, n_cycles=1)
    #     print(c, r["delivered_vt_ml"])
    # psv.DT = 0.01
    # psv.DT = 0.001
    # r = generate_breath_cycles({**RDS_PARAMS, "pressure_support_cmH2O": 10.0}, n_cycles=1)
    # psv.DT = 0.01  # restore
    ...
    # flow = r["flow"]
    # nonzero = np.nonzero(np.abs(flow) > 1e-6)[0]
    # print("first/last nonzero index:", nonzero[0], nonzero[-1])
    # print("duration of inspiratory+early-decay window (s):", (nonzero[-1]-nonzero[0]) * 0.001)
    # print("peak flow:", flow.max(), "at index", flow.argmax())
    # print("flow at cycle-out (last nonzero before long zero run):", flow[nonzero[-1]])
    # import generator.psv_generator as psv
    # original_dt = psv.DT
    # psv.DT = 0.001   # 10x finer, temporary
    # r = generate_breath_cycles({**RDS_PARAMS, "pressure_support_cmH2O": 10.0}, n_cycles=1)
    # psv.DT = original_dt
    # print(len(r["time"]), r["delivered_vt_ml"])
    # print(r["flow"][r["flow"] != 0])
    # from generator.psv_generator import DT
    # print("DT:", DT)

    # r = generate_breath_cycles({**RDS_PARAMS, "pressure_support_cmH2O": 10.0}, n_cycles=1)
    # print("total samples in run:", len(r["time"]))
    # print("time:", r["time"])
    # print("flow:", r["flow"])
    # print("volume:", r["volume"])
    # for fct in (0.15, 0.10, 0.07, 0.05, 0.03):
    #     p = {**RDS_PARAMS, "pressure_support_cmH2O": 10.0, "flow_cycle_threshold": fct}
    #     r = generate_breath_cycles(p, n_cycles=5)
    #     print(fct, r["delivered_vt_ml"], r["breath_dyssynchrony_labels"].count("flow_starvation"))
    # for ps in (8.0, 10.0, 14.0, 18.0):
    #     p = {**RDS_PARAMS, "pressure_support_cmH2O": ps}
    #     r = generate_breath_cycles(p, n_cycles=5)
    #     print(ps, r["delivered_vt_ml"], r["breath_dyssynchrony_labels"].count("flow_starvation"))
